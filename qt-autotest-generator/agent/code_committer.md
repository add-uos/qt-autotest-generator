...
```
test: add autotests for <project> batch <batch_no> (<batch_done>/<batch_total> classes)

Generated by qt-autotest-generator skill.
Batch <batch_no>: <batch_classes_str>
...
| 必含字段 | 校验规则 |
|---------|---------|
| 标题行 | 以 `test: add autotests for` 开头，含 `batch <N>`，含 `(<done>/<total> classes)` |
| 基线 commit | `Baseline: <branch> @ <short-sha> "<title>" (<date>)` 完整 |
| 本批次类列表 | `Batch <N>:` 行存在且非空 |
...
- **提交信息必须含基线 commit、批次类列表、覆盖率摘要、累计统计**：缺一不可
