...
**提交信息格式**（强制模板与正则以 [`resources/references/commit-msg-format.md`](../resources/references/commit-msg-format.md) §1 为单一权威来源；本文档与 `self_checker(commit_check=true)` 均引用该文档，避免生成与校验不同步）：
...
> ⚠️ 旧版本标题行缺 `batch <N>` 后缀、本批次类列表用 `Classes:` 而非 `Batch <N>:`，与 `self_checker` 正则不符会被判 `FAIL_TITLE` / `FAIL_BATCH_LINE`。生成时严格按 `commit-msg-format.md` §1 模板，**不要回退旧格式**。

### 6. 提交信息格式自校验

提交前对生成的提交信息做格式自校验（**提交时自检**，与提交后 `self_checker(commit_check=true)` 的提交规范自检互补）。**正则以 [`resources/references/commit-msg-format.md`](../resources/references/commit-msg-format.md) §2 为单一权威来源**，本步骤直接套用该文档 §3 校验脚本（传入临时生成的 commit message 文件路径）。
