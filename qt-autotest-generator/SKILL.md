...
compatibility:
  required_mcp_any_of:
    - name: gitnexus-mcp
      purpose: "GitNexus 代码图谱 MCP（cypher/list_repos/context），仓库级索引；硬门禁，仓库未索引不执行"
  env_vars:
    - name: QTAG_MCP_URL
      default: "https://codegraph.uniontech.com/api/mcp"
      description: "GitNexus MCP HTTP 端点，mcp-scan.py 使用"
    - name: QTAG_MCP_API_KEY
      default: ""
      description: "MCP HTTP 端点认证头 X-API-Key 的值；按实际部署配置"
    - name: QTAG_MCP_HEADERS
      default: ""
      description: "MCP HTTP 额外请求头，JSON 字符串（如 '{\"Authorization\":\"Basic xxx\"}'）；内网部署默认 Basic 认证已内置"
---

# Qt Autotest Generator

基于 **GitNexus 代码图谱 MCP** 的 Qt CMake 项目单元测试技能。图谱只当索引（类/方法/调用边），方法体/复杂度/签名/宏扫描来自本地仓库（`--repo-root`，双源架构）；分步指令在 **`references/`**，执行前须 **`Read`** 对应文件。

| 模式 | 何时用 | 主入口 |
|------|--------|--------|
| **Mode 0 · Dev Preflight** | 本地开发中、有未 push 代码、需确认图谱漂移 | 下文 + `Read references/dev-preflight.md` |
...
- **知识图谱 MCP 硬门禁**：仓库未在 GitNexus 索引不执行，不降级到文件扫描/LSP（`list_repos` 确认；见 `references/gitnexus-guide.md`）。
- **不修源码**：疑似源码缺陷只标红交还用户。
...
- **Mode 0**：dev preflight、开发预检、dev 模式、图谱漂移、检查图谱同步、同步图谱、sync graph、unpushed、未推送、mode 0（注意：普通"生成单测/写测试"归 Mode 2，即使用户在本地开发，不要因"本地"字样误路由到 Mode 0）
...
1. **`Read`** `references/dev-preflight.md` → 完整执行索引确认（list_repos）+ 漂移检查（check_drift）
2. 完成后**自动路由**：
   - `.ut-inventory.json` 不存在 → Mode 1
   - `.ut-inventory.json` 存在 → Mode 2（自动走 reconcile）

Mode 0 **不生成测试代码、不编译、不运行**，只确认 GitNexus 已索引目标仓库、量化本地 HEAD 与图谱 lastCommit 的漂移。
...
| 依赖追踪 | `references/dependency-tracer.md` | 读 inventory 的 is_gui、图谱 CALLS 出向、stub 决策、CMake 目录（MCP 详见 `references/gitnexus-guide.md`） |
...
| MCP 工具 | GitNexus：`cypher`（任意图查询）、`list_repos`（确认索引/base_sha）、`context`（降级内容读取）+ `scripts/mcp-scan.py`（fetch/scan/extract-branches/test-mapping）—— **项目源码理解的唯一来源，禁止 read/grep 直读源文件** |
...
- 用 Qt Test / Catch2 框架
- GitNexus 图谱未 ready（仓库未索引）就开始生成
- MCP 端点未配置（QTAG_MCP_URL / --mcp-url）或指向非 GitNexus 服务
- 未编译通过就报完成
- 从网络下载 stub-ext
...
- 测试文件声明了分支清单但未用 extract-branches 反查真实源码分支（声明 < 实际即漏测）
- 本地 HEAD 领先图谱 lastCommit（check_drift 报漂移）仍继续生成——必须硬终止并等待平台同步图谱（GitNexus 无本地索引，不可自行补索引）
...
□ Mode 0：已 Read references/dev-preflight.md；list_repos 确认仓库已索引、check_drift 漂移已确认（或等待平台同步）；已设置 mode_0_active 标志
...
□ MCP 端点已解析（GitNexus 单栈，未索引硬终止不回退）
□ 项目源码理解只走图谱：方法体用 mcp-scan.py（图谱定位+本地切片）、调用链用 cypher CALLS 边、类与方法用 cypher / fetch inventory、IMPORTS 用 cypher 关系查询；未用 `read`/`grep` 直读项目源码
□ 逐类闭环：每类走 依赖追踪 → 测试生成 → 编译验证 → 自检（类列表与 level 来自 inventory）
□ 白盒反查：分支清单已用 extract-branches 取真实源码分支做差集，无 `BRANCH_NOT_MAPPED` 漏报
