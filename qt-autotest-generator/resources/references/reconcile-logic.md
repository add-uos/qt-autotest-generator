...
> 每次技能触发后必先执行 reconcile，判断源码是否变更，再决定执行哪个 phase。

## 流程

```
1. 读 {test_dir}/.ut-session.json（不存在 → 首次运行）
2. 首次运行：
   ...
   c. 与 session.baseline_commit 比较
    ...
    f. 分支切换检测：git branch --show-current 与 session 记录的分支比较，
       若不同 → 强制刷新索引后重新对账：
         ...
         · 重新对账前，遍历 session.classes 检查每个类的 file_path 是否在当前分支仍存在
         ...
         · 更新 session.branch + session.baseline_commit + session.stale_classes
```
...
4. 记录 `session.stale_classes` 列表
5. 新分支重新走类分析 → 新增/变更的类正常闭环
6. 更新 `session.branch` + `session.baseline_commit` + `session.stale_classes`
