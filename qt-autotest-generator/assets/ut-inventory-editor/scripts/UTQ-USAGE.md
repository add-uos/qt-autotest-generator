...
> 脚本位置: `scripts/utq.py`（在 ut-inventory-editor 目录下运行，或用 `-P` 指定项目目录）
...
```bash
python3 scripts/utq.py -P mcp-projects/deepin-image-viewer top 20      # 按分数
python3 scripts/utq.py -P mcp-projects/deepin-image-viewer todo --level high   # 只看 high 级
```

### S3. 定点补某个模块/文件

```bash
python3 scripts/utq.py -P mcp-projects/deepin-image-viewer file imageeditcontroller
python3 scripts/utq.py -P mcp-projects/deepin-image-viewer file pathviewproxymodel --level high
```
...
```bash
python3 scripts/utq.py -P mcp-projects/deepin-image-viewer class ImageEditController
```

### S5. 写测试前查参考：已有用例长什么样

```bash
python3 scripts/utq.py -P mcp-projects/deepin-image-viewer info saveComposite   # 单函数完整信息
python3 scripts/utq.py -P mcp-projects/deepin-image-viewer covered --kw detectImageFormat --show-cases
```
...
```bash
python3 scripts/utq.py -P mcp-projects/deepin-image-viewer files --sort pct --limit 10   # 覆盖率最低的文件
python3 scripts/utq.py -P mcp-projects/deepin-image-viewer classes --limit 10           # 未测最多的类
```

### S7. 弱覆盖：已测但用例太少

```bash
python3 scripts/utq.py -P mcp-projects/deepin-image-viewer weak
```
...
```bash
python3 scripts/utq.py -P mcp-projects/deepin-image-viewer by-test-file ut_filecontrol
```
...
```bash
python3 scripts/utq.py -P mcp-projects/deepin-image-viewer pending     # 待人工定级 20 条
python3 scripts/utq.py -P mcp-projects/deepin-image-viewer exempt      # 豁免清单+原因
```

### S10. 导出任务包（喂给子代理批量写）

```bash
python3 scripts/utq.py -P mcp-projects/deepin-image-viewer export --level high > /tmp/tasks.json
python3 scripts/utq.py -P mcp-projects/deepin-image-viewer export --file unionimage --limit 10 | tail -n +3
```
