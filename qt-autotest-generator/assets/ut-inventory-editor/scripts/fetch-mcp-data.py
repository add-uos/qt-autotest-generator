...
def main():
    ...
    parser.add_argument("--branch", default=None,
                        help="Git 分支（默认自查 scripts/project-branches.json，"
                             "查不到则 fallback master）")
    parser.add_argument("--org", default=None,
                        help="GitHub org（默认 linuxdeepin）")
    ...
    # 注入项目远端/分支信息（新数据自带；供编辑器 GitHub 面板直接使用）
    inventory["git"] = lookup_git_info(args.project, args.branch, args.org)
    print(f"🌿 git: {inventory['git']['branch']} ({inventory['git']['branch_source']})"
          f" → {inventory['git']['remote']}")
