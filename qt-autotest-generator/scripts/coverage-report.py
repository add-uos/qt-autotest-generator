...
def compute_tiered_coverage(inventory_path, coverage_info, output_json=None,
    ...
    返回: 结果 dict（by_level / total / unmatched / uncovered_functions / class）。
    """
    ...
    if json_output:
        ...
        if output_json:
            open(output_json, "w", encoding="utf-8").write(out)
    ...
    # 人类可读汇总
    print(f"\n项目: {inv.get('project','?')}"
          + (f"  | 类: {class_name}" if class_name else ""))
    ...
    if output_json:
        json.dump(result, open(output_json, "w", encoding="utf-8"), indent=2, ensure_ascii=False)
        print(f"\n汇总 JSON 已写入: {output_json}")
...
def run_coverage_by_level(inventory, coverage_info, output_json):
    """计算分级覆盖率（直接调用 compute_tiered_coverage，不再走 subprocess）并返回结果 dict。"""
    if not os.path.exists(coverage_info):
...
def main_no_exit(argv=None):
    ...
    # Keep a filtered copy for coverage-by-level (which expects a .info)
