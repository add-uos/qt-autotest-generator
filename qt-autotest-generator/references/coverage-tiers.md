...
方法按重要性分为三级，每级有差异化的覆盖率门禁。分类依据来自 Mode 1（函数重要性探测，详见 `references/inventory.md`）产出的 `.ut-inventory.json`。
...
| 基础 | low | ≥ 60% | — | 100% | 构造/析构、简单 getter/setter、运算符重载（门禁同 mid） |
...
| low | 触发增量补全 | 不检查 | 触发增量补全（hard gate） |

## 产出文件

- 产出路径：`${test_dir}/.ut-inventory.json`
- 格式详见：`references/inventory-schema.md`
- 由 `scripts/mcp-scan.py` 从预采集的 MCP 图谱数据生成
- MCP 数据采集格式见：`scripts/mcp-scan.py`
