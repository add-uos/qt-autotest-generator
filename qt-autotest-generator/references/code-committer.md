...
> **首选方式**：跑 `scripts/mode2-ops.py commit`，脚本从 classes_status 统计本批次/累计数据，
...
> python3 ${SKILL_DIR}/scripts/mode2-ops.py commit --status-file status.json [--git-dir ${PROJECT_PATH}]
...
- **仍有下一批次**：返回 `test-writer.md` §5 逐类闭环处理下一批次；**不在此处生成报告**
- **全部批次提交完成**（无下一批次）：进入**最终退出前报告阶段**（见 `test-writer.md` §9），统一生成一次 Mode 3 覆盖率报告 + Mode 5 缺陷导出
