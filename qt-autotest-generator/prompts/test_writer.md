...
**若 `.ut-inventory.json` 不存在** → 先执行 Mode 1（`Read prompts/inventory.md`）。
...
若 `{test_dir}/` 不存在 → `Read prompts/framework_builder.md`
...
本批次所有类自检通过后 → `Read prompts/code_committer.md`
