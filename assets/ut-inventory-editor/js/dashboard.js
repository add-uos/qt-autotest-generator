...
function renderDashGrid(){const l=dashFiltered();$('#dash-count').textContent=l.length+' 项目';const el=$('#dash-overview');if(!l.length){el.innerHTML='<div class="dash-empty" style="grid-column:1/-1"><i data-lucide="layout-grid" class="icon"></i><div style="margin-top:8px">无项目数据 — 点击右上「刷新」或「导入」</div></div>';return}el.innerHTML=l.map(p=>{const s=p.stats,cov=s.testable?Math.round(s.withCover/s.testable*100):0,gapCls=s.noCoverHigh>0?'risk':'ok',hm=s.high+s.mid;return`<div class="dash-card" data-proj="${p.name}"><div class="dash-card-header"><span class="size-badge ${(p.size||'').toLowerCase()}">${p.size||'?'}</span><span class="pname" title="${p.name}">${p.name}</span><span class="gh-link" data-gh="${p.name}/blob/${p.branch}" title="GitHub 仓库"><i data-lucide="github" class="icon"></i></span></div><div class="bar-track" title="高+中 ${hm} / 可测 ${s.testable}"><div class="bar-fill ${s.high>0?'warn':'info'}" style="width:${s.testable?Math.round(hm/s.testable*100):0}%"></div></div><div class="dash-levrow"><span class="level-badge level-high">🟢 ${s.high}</span><span class="level-badge level-mid">⚖ ${s.mid}</span><span class="level-badge level-low">💤 ${s.low}</span>${s.reviewPending?`<span class="level-badge level-exempt">⏳ ${s.reviewPending}</span>`:''}</div><div class="dash-gap ${gapCls}">${s.noCoverHigh>0?'⚠':'✓'} 高优无覆盖: <span class="n">${s.noCoverHigh}</span></div><div class="dash-cover"><span>覆盖 ${s.withCover}/${s.testable}</span><span>${cov}%</span></div><div class="dash-cover"><div class="bar-track" title="已覆盖 ${s.withCover} / 可测 ${s.testable} ≈ ${cov}%" style="flex:1"><div class="bar-fill" style="width:${cov}%"></div></div></div>${p.generatedAt?`<div class="dash-meta"><span>${p.generatedAt.slice(0,10)||''}</span><span>${p.baseSha?'SHA '+p.baseSha:''}</span></div>`:''}${renderTestCardRow(p.name)}<div class="test-run-row"><div class="run-dd-wrap"><button class="test-run-btn" data-run="${p.name}"><i data-lucide="play" class="icon"></i>运行</button><div class="export-dropdown run-dd" id="run-dd-${p.name}"><div class="export-dropdown-item" data-run-mode="full" data-run-name="${p.name}">🔧 完整流程 (配置→编译→测试→覆盖)</div><div class="export-dropdown-item" data-run-mode="test-only" data-run-name="${p.name}">⚡ 仅测试 (跳过编译)</div><div class="export-dropdown-item" data-run-mode="test+coverage" data-run-name="${p.name}">📊 测试+覆盖</div><div class="export-dropdown-item" data-run-mode="build+test" data-run-name="${p.name}">🔨 编译+测试</div><div class="export-dropdown-item" data-run-mode="coverage-only" data-run-name="${p.name}">📈 仅覆盖率</div><div class="export-dropdown-item" data-run-mode="script" data-run-name="${p.name}">📜 项目脚本 (run-ut.sh)</div></div></div></div></div>`}).join('')}
...
function renderTestCardRow(name){const st=S.dash.testRunStatus&&S.dash.testRunStatus[name];if(st&&(st.state==='running'||st.state==='queued')){const pct=st.progress?' '+st.progress:'';return`<div class="test-status-card"><div class="spin"></div>${st.label||st.phase||'运行中'}${pct} · ${st.elapsed||0}s</div>`}const t=S.dash.testResults[name];if(st&&st.state==='done'){const t2=S.dash.testResults[name];if(t2&&t2.available){const s=t2.test_summary||{};const dot=s.failed>0?'ng':'ok';return`<div class="test-card-row"><span class="tc-dot ${dot}"></span><span class="tc-seg seg-a">✓ 测试 ${s.passed||0}/${s.total||0}</span><span class="tc-seg">行 ${s.line_coverage||'—'}</span><span class="tc-seg">函数 ${s.function_coverage||'—'}</span></div><div class="detail-small tc-time"><span class="tc-dot ${dot}"></span>统计时间 ${t.last_run?t.last_run.slice(0,19):''}</div>`}}if(!t||!t.available)return'';const s=t.test_summary||{};const dot=s.failed>0?'ng':'ok';return`<div class="test-card-row"><span class="tc-dot ${dot}"></span><span class="tc-seg seg-a">用例 ${s.passed||0}/${s.total||0}</span><span class="tc-seg">行 ${s.line_coverage||'—'}</span><span class="tc-seg">函数 ${s.function_coverage||'—'}</span></div><div class="detail-small tc-time"><span class="tc-dot ${dot}"></span>统计时间 ${t.last_run?t.last_run.slice(0,19):''}</div>`}
...
async function loadDashProjects(){try{const r=await dashApi('/api/projects');const d=await r.json();S.dash.projects=(d.projects||[]).map(normDashProject);saveDashCache();saveDashSnapshot();renderDash()}catch(e){toast('拉取项目列表失败: '+e.message,'err')}preloadTestResults()}

// 批量预取全部项目测试结果（并发 6），完成后重渲染网格；已缓存的跳过
let _preloadBusy=false;
async function preloadTestResults(){
  if(_preloadBusy||!S.dash.projects.length)return;
  _preloadBusy=true;
  try{
    const names=S.dash.projects.map(p=>p.name).filter(n=>!S.dash.testResults[n]);
    let i=0;
    const worker=async()=>{while(i<names.length){await fetchTestResults(names[i++]).catch(()=>{});}};
    await Promise.all(Array.from({length:Math.min(6,names.length)},worker));
    renderDashGridIfVisible();
  }finally{_preloadBusy=false}
}
    ...
    if(runBtn){
      ...
      if(dd){
        ...
        if(dd.classList.contains('show')){
          // 防溢出：优先向右展开；按钮右侧放不下时改向左（对齐右边缘）
          const r=runBtn.getBoundingClientRect();
          const vw=document.documentElement.clientWidth;
          dd.classList.toggle('align-right', r.left+dd.offsetWidth>vw-8);
        }
      }
      return;
    }
...
function renderDashGridIfVisible(){
  if(S.view==='dashboard'&&S.dash.sub==='overview'){renderDashGrid();try{lucide.createIcons()}catch(e){}}
}
