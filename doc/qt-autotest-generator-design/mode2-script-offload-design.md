...
**已实现**：`scripts/self-check-structural.py` + `test/test_self_check_structural.py`（48 用例）
...
| env | 硬编码绝对路径（排除 QTemporaryDir）/ qputenv-qunsetenv 不平衡 / 真实外部资源（排除 stub） | HARDCODED_PATH / ENV_UNBALANCED / REAL_EXTERNAL_CALL |
...
**已实现**：`scripts/update-usecase-count.py` + `test/test_update_usecase_count.py`（18 用例）
...
**已实现**：`scripts/compose-commit.py` + `test/test_compose_commit.py`（15 用例）

固化 code-committer.md §5 的纯模板渲染：从 classes_status 统计本批次/累计数据 → 按 git-commit-workflow test 类型 Log/Influence 格式生成提交信息到 stdout。

**实现要点**：
- 输入：`--status-file` 读 JSON（模型把内存变量 dump 出来：classes_status/batch_classes/baseline_commit/branch_name/project_name/test_dir/pms_no/issue_no）
- `--git-dir` 提供时用 `git log` 查 baseline title/date；不提供时占位
...
| P1 | §2.3 self-check-structural.py | **✅ 已落地**（`scripts/self-check-structural.py` + 48 单测；sample 验证如实报出 15 LOW_ASSERT；reference `self-checker.md` 已接入首选方式） |
| P2 | §2.4 update-usecase-count.py | **✅ 已落地**（`scripts/update-usecase-count.py` + 18 单测；sample 15 用例正确分配到 9 方法；reference `test-writer.md §6` 已接入首选方式） |
| P2 | §2.5 compose-commit.py | **✅ 已落地**（`scripts/compose-commit.py` + 15 单测；端到端验证多类多状态；reference `code-committer.md §5` 已接入首选方式） |
